...
public class Multiply {
    public static class MapperM extends Mapper<Object, Text, IntWritable, Elem> {
        private Map<Integer, Elem> localMap;
        ...
        public void map(Object key, Text value, Context context) throws IOException, InterruptedException {
            ...
            if (!localMap.containsKey(j)) {
                localMap.put(j, new Elem((short) 0, i, v));
            } else {
                Elem existing = localMap.get(j);
                existing.value += v;
                localMap.put(j, existing);
            }
        }

        @Override
        protected void cleanup(Context context) throws IOException, InterruptedException {
            for (Map.Entry<Integer, Elem> entry : localMap.entrySet()) {
    ...
    public static class MapperN extends Mapper<Object, Text, IntWritable, Elem> {
        ...
        public void map(Object key, Text value, Context context) throws IOException, InterruptedException {
            ...
            if (!localMap.containsKey(i)) {
                localMap.put(i, new Elem((short) 1, j, v));
            } else {
                Elem existing = localMap.get(i);
                existing.value += v; 
                localMap.put(i, existing);
            }
        }
